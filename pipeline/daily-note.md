...
### reStructuredText 원문의 함정 — `<` 하나에 수천 자가 사라진다

검증기는 HTML 태그를 `<[^>]+>`로 지운다. 그래서 **`.rst` 원문의 코드 예시에 `if x < 10:`
같은 줄이 있으면**, 그 `<`부터 한참 뒤의 `>`까지가 태그로 오인되어 통째로 지워진다
(2026-09-11 실행에서 PEP 8 본문 7,791자가 이렇게 사라졌다 — 고른 문장 두 개가 그 구간에
있어서 `MISS`가 났다). 원문에 분명히 있는 문장인데 `MISS`가 나면 이 경우를 의심한다.
...
python3 - <<'PY'
import re
raw = open('원문.rst', encoding='utf-8').read()
for m in re.finditer(r"<[^>]+>", raw):
    if len(m.group(0)) > 40:
        print(len(m.group(0)), '줄', raw[:m.start()].count('\n') + 1)
PY
```
